...
class HCFlowSRModel(BaseModel):
    ...
    def optimize_parameters(self, step):
        ...
        if (step % self.D_update_ratio == 0 and step > self.D_init_iters) or (not self.cri_gan):
            ...
            # reverse flow (optimize NLL loss and HR_mod32 loss seperately (takes less memory and more time, slightly better results))
    ...
    def test(self):
        ...
        with torch.no_grad():
            if self.real_H is None:
                ...
                self.fake_L_from_H, nll = self.netG(hr=self.real_H, lr=self.var_L, u=None, reverse=False,
                                                    training=False)
