...
import logging
import torch
import archs.discriminator_vgg_arch as SRGAN_arch

logger = logging.getLogger('base')


def find_model_using_name(model_name, downsample_method_name=None):
    model_filename = "archs." + model_name + "_arch"
    modellib = importlib.import_module(model_filename)

    model = None

    target_model_name = downsample_method_name if downsample_method_name is not None else model_name
...
def define_Downsample(opt):
    which_model_Downsample = opt['network_Downsample']['which_model_Downsample']
    which_method = opt['network_Downsample']['which_method']
    Arch = find_model_using_name(which_model_Downsample, downsample_method_name=which_method)
    netG = Arch(opt=opt)

    return netG


def define_Flow(opt, step=None):
