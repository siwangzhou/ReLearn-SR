...
class HCFLowRescalingModel(BaseModel):
    def __init__(self, opt, step):
        ...
        if self.is_train:
            ...
            if train_opt['pixel_weight_hr'] > 0:
                ...
                logger.info('Remove HR_mod32 pixel loss.')
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            self.fake_L_from_H, fake_z1, fake_z2 = self.netG(hr=self.real_H, lr=self.var_L, u=None, reverse=False,
                                                             training=False)
            ...
            for heat in self.heats:
                for sample in range(self.n_sample):
                    ...
                    self.fake_H[(heat, sample)] = self.netG(lr=self.fake_L_from_H,
    ...
    def my_test(self, LR):
        ...
        for heat in self.heats:
            # z = self.get_z(heat, seed=1, batch_size=self.var_L.shape[0], lr_shape=self.var_L.shape)
            fake_H = self.netG(lr=LR,
                               z=None, u=None, eps_std=heat, reverse=True, training=False)

            # hr->lr+z, calculate nll
            fake_L_from_SR, fake_z1, fake_z2 = self.netG(hr=fake_H, lr=None, u=None, reverse=False,
                                                         training=False)
    ...
    # f(g(y+delta_y))
    def my_test_2(self, HR):
        self.netG.eval()
        # hr->lr+z, calculate nll
        fake_L_from_SR, fake_z1, fake_z2 = self.netG(hr=HR, lr=None, u=None, reverse=False,
                                                     training=False)
        fake_L_from_SR = self.Quantization(fake_L_from_SR)

        # lr+z->hr
        fake_H = self.netG(lr=fake_L_from_SR,
                           z=None, u=None, eps_std=1.0, reverse=True, training=False)

        self.netG.train()
        return fake_H, fake_L_from_SR

    # g(f(x)+delta_y)
    def my_test_3(self, LR, delta_y):
        ...
        # hr->lr+z, calculate nll
        fake_L_from_SR, fake_z1, fake_z2 = self.netG(hr=fake_H + delta_y, lr=None, u=None, reverse=False,
                                                     training=False)
        # fake_L_from_SR = self.Quantization(fake_L_from_SR)
