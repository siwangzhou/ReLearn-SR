...
def random_batch_kernel(batch, l=21, sig_min=0.2, sig_max=4.0, rate_iso=1.0, scale=3, tensor=True):
    ...
    for i in range(batch):
        batch_kernel[i, :shifted_l, :shifted_l], batch_sigma[i, :] = \
            random_gaussian_kernel(l=shifted_l, sig_min=sig_min, sig_max=sig_max, rate_iso=rate_iso, scale=scale,
                                   tensor=False)
    if tensor:
...
def stable_batch_kernel(batch, l=21, sig=2.6, sig1=2.6, sig2=2.6, theta=0, rate_iso=1.0, scale=3, tensor=True):
    ...
    for i in range(batch):
        batch_kernel[i, :shifted_l, :shifted_l], batch_sigma[i, :] = \
            stable_gaussian_kernel(l=shifted_l, sig=sig, sig1=sig1, sig2=sig2, theta=theta, rate_iso=rate_iso,
                                   scale=scale, tensor=False)
    if tensor:
        return torch.FloatTensor(batch_kernel), torch.FloatTensor(batch_sigma)
    else:
        return batch_kernel, batch_sigma
...
def stable_batch_kernel_SV_mode(batch, img_H=250, img_W=250, divide_H=1, divide_W=1, sv_mode=0, l=21, sig=2.6, sig1=2.6,
                                sig2=2.6, theta=0, rate_iso=1.0, scale=3, tensor=True):
...
def b_GaussianNoising(tensor, sigma, mean=0.0, noise_size=None, min=0.0, max=1.0):
    if noise_size is None:
        ...
        size = noise_size
    noise = torch.mul(sigma.new_tensor(np.random.normal(loc=mean, scale=1.0, size=size)),
...
class BatchSRKernel(object):
    ...
    def __call__(self, random, batch, tensor=False):
        if random == True:  # random kernel
            return random_batch_kernel(batch, l=self.l, sig_min=self.sig_min, sig_max=self.sig_max,
                                       rate_iso=self.rate_iso,
...
class BatchSRKernel_SV(object):
    def __init__(self, l=21, sig=2.6, sig1=2.6, sig2=2.6, theta=0, sig_min=0.2, sig_max=4.0, rate_iso=1.0, scale=3,
                 divide_H=1, divide_W=1, sv_mode=0):
        self.l = l
        self.sig = sig
        self.sig1 = sig1
        self.sig2 = sig2
    ...
    def __call__(self, random, batch, img_H, img_W, tensor=False):
        return stable_batch_kernel_SV_mode(batch, img_H=img_H, img_W=img_W, divide_H=self.divide_H,
                                           divide_W=self.divide_W, sv_mode=self.sv_mode, l=self.l, sig=self.sig,
                                           sig1=self.sig1, sig2=self.sig2, theta=self.theta,
                                           rate_iso=self.rate_iso, scale=self.scale, tensor=tensor)
...
class PCADecoder(object):
    ...
    def __call__(self, batch_kernel_map):
        ...
        return torch.bmm(batch_kernel_map.unsqueeze(1), self.weight.expand((B,) + self.size)).view(
            (B, int(self.size[1] ** 0.5), int(self.size[1] ** 0.5)))
...
class SRMDPreprocessing(object):
    def __init__(self, scale, random, l=21, add_noise=False, device=torch.device('cuda'), sig=2.6, sig1=2.6, sig2=2.6,
                 theta=0,
                 sig_min=0.2, sig_max=4.0, rate_iso=1.0, rate_cln=0.2, noise_high=0.05882, is_training=False,
                 sv_mode=0):
...
def save_perturbation(delta, save_path, normalize=True):

    # 将扰动转换为 NumPy 数组
    delta = delta.detach().cpu().numpy()[0]  # 假设 delta 形状为 [1, C, H, W]

    # 取绝对值并合并通道，假设是3通道的扰动
    delta_abs = np.abs(delta)  # 获取绝对值
    delta_abs = np.transpose(delta_abs, (1, 2, 0))  # 从 CHW 转为 HWC

    # 如果需要标准化到 [0, 255] 范围
    if normalize:
        # 标准化到 [0, 1] 范围
        delta_abs = (delta_abs - np.min(delta_abs)) / (np.max(delta_abs) - np.min(delta_abs))
        # 标准化到 [0, 255] 范围
        delta_abs = (delta_abs * 255).astype(np.uint8)

    # 保存扰动图像
    cv2.imwrite(save_path, delta_abs)  # 使用 OpenCV 保存图像
...
def calculate_psnr_ssim(img1, img2, crop_border=0,calculate_y=True):
    if img2.shape[2] != 3:
